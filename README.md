...
- `send-markdown`：从messages.json中随机选择一条Markdown消息发送到指定dialog_id，可选N秒后自动删除。参数：dialog_id, --delete-after。
...
- **如何使用Markdown格式发送消息？**
  使用 `send-markdown` 命令，该命令会从 `messages.json` 文件中随机选择一条消息并以Markdown格式发送：
  ```bash
  uv run main.py -a fury send-markdown 77893
  ```
- **messages.json文件格式要求是什么？**
  `messages.json` 应该是一个包含字符串数组的JSON文件，每个字符串都是一条Markdown格式的消息。示例：
  ```json
  [
    "**Hello**! This is a *test* message.",
    "Another `test` message with [a link](https://example.com).",
